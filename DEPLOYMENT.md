...
In the Supabase SQL editor (or via `supabase db push`), run in order: `supabase_schema.sql`, then `supabase_schema_v2.sql`, then `supabase_schema_v3.sql`, then `supabase_schema_v4.sql`, then `supabase_schema_v5.sql`, then `supabase_schema_v6.sql`, then `supabase_schema_v7.sql`, then `supabase_schema_v8.sql`, then `supabase_schema_v9.sql`, then `supabase_schema_v10.sql`, then `supabase_schema_v11.sql`, then `supabase_schema_v12.sql`, then `supabase_schema_v13.sql`, then `supabase_schema_v14.sql`. All are safe to re-run (guarded with `IF NOT EXISTS` / `ON CONFLICT`, or a `drop policy if exists` for v9/v12/v14).

`supabase_schema_v14.sql` adds `app_links` (two rows, `qualification_form`/`booking_meeting`, seeded with a null `value` — paste the real URLs into Settings → Forms & Links after deploying) and `leads.last_qualification_form_submission_at` (used to dedupe the new qualification-form-submission comm_log entries described in step 11 below).
...
## 10. Sidebar navigation + card grid + fake-lead cleanup

Sales Pipeline's flat table and separate "Cold Leads" table are gone (see step 1's `supabase_schema_v13.sql` note for the stage-rename migration that went with this). The list layer has been rebuilt twice since: first as a horizontally-scrolling Kanban board, then as a 3-level drill-down (stage menu → stage's leads → a `/leads/:id` full page), and now as the current **sidebar + grid + modal** structure, which replaces both of those. Stat cards and the filter bar are unchanged and stay visible above the list; only the list/detail layer underneath them changed shape:

- **Default view:** the grid shows the "New Lead" stage's leads immediately — there's no separate stage-menu screen to pass through first.
- **☰ sidebar:** a button (left of the filter bar) opens a slide-in panel listing all 9 stages with filter-respecting counts, the current stage highlighted. Clicking a stage switches the grid to it and closes the sidebar. This is the only way to change which stage the grid is showing — the old dedicated "stage menu" page and the old per-row `<select>` are both gone.
- **Card grid:** leads in the selected stage render as a 2-column grid (collapses to 1 column under 640px) — same card content/styling as every prior pass (business name, contact, qualification badge with override marker, value, source pill, cold tag).
- **Lead detail:** clicking a card opens it in a modal on top of the grid (qualification score + override, Templates button, comms history, stage-change `<select>`, edit/delete) — not a page navigation. Closing it (X, clicking outside, or Escape) leaves the grid exactly as it was; the stage you were viewing is never lost. **There is intentionally no deep-linkable URL per lead anymore** — the `/leads/:id` routing, the `vercel.json` rewrite that supported it, and the matching `serve.json` rewrite have all been removed along with it (this was an explicit tradeoff, not an oversight — flag it if a deep-linkable lead URL turns out to matter later, since reintroducing it would mean redoing this same routing work).

Removed along with the old board/drill-down: drag-and-drop stage changes and the routing infrastructure above (no columns to drag between, and no more page to navigate to — stage changes and everything else now happen inside the modal), plus the generic multi-select "Delete Selected" bulk-delete bar from an even earlier pass (no table row left to host a checkbox column). Single-lead delete is unchanged, now reached via the modal. The **Settings → 🧹 Data Cleanup** section is the closest replacement for reviewed bulk deletion going forward — see below.
...
## 11. Settings sidebar, Templates manager, Link library, and a real Part 5 investigation

**Settings sidebar** — the old two-column accordion (`.settings-grid`, click-a-header-to-expand) is gone, replaced by the exact same ☰ sidebar pattern the Pipeline page already used: one category visible at a time, a slide-in panel listing all of them (now 11 — the original 9 plus **Templates** and **Forms & Links** below), the active one highlighted. All the original categories' content/behavior is byte-for-byte unchanged; only the outer accordion chrome was removed. `toggleSettingsAccordion()` is gone; `showSettingsCategory()`/`openSettingsSidebar()`/`selectSettingsCategory()` reuse the Pipeline sidebar's own `#sidebarRoot` mount point and CSS classes (safe, since only one tab is ever visible at a time, so the two sidebars never overlap).

**Templates manager** (Settings → 📄 Templates) — the `templates` table (schema v12) was seeded with 16 placeholder rows and had no editing UI at all; this adds one. Click a template → edit its real subject/body → Save writes straight to `templates` (same open, anon-key RLS as before — no new admin-password gate, matching what v12's own comment already anticipated: "future admin UI," which this is). Two insert helpers in the body editor:
- **Merge fields** (`{{first_name}}` etc.) — same 6 keys `send-template`/the send preview already fill, inserted at the cursor.
- **Stored links** — wraps the selected text (or falls back to the link's label) in `[text](key)`, e.g. `[click here](qualification_form)`. This is a markdown-style extension of the single-marker pattern `send-questionnaire-email` already used (`_shared/gmail.ts`'s `bodyWithLink`/`LINK_MARKER`) — generalized to multiple, independently-chosen links anywhere in the body instead of one fixed marker. `bodyWithLinks()` (new, same file) resolves every `[label](key)` span against a key→URL map at send time, escaping first so a raw `<a>` tag typed into the body can't leak through unescaped and so a link with no URL saved yet degrades to plain text instead of breaking the send. `send-template` now calls this instead of the old plain `textToHtmlBody`, fetching the map fresh from `app_links` on every send.

**Forms & Links** (Settings → 📋 Forms & Links) — two plain URL fields, Qualification Form and Booking Meeting, backed by the new `app_links` table (step 1). Global, not per-lead. This is the only place those two URLs live; the Templates editor's "insert link" dropdown reads directly from this table.

**Lead modal cleanup** — removed the Nurture / Book Call / Recap / Follow Up / Reactivate buttons per the spec's explicit list and stated rationale (they duplicated what Templates now does). **Flagging a real side effect the spec's stated rationale doesn't cover:** `confirmBookCall` (behind the removed "Book Call" button) did more than draft an email — it also set the lead's stage to `Discovery Booked`, set `discovery_call_date`, and was the **only** call site for `triggerMeetingPrepBrief()`, the meeting-prep-brief generator. Removing the button makes that generator unreachable from the UI. I left `openBookCallModal`/`confirmBookCall`/`triggerMeetingPrepBrief` in `index.html` rather than deleting them — they're dead code today, but keeping them intact means restoring this (a button, a different trigger point, whatever's wanted) doesn't require reconstructing the logic. The other four removed buttons had no comparable side effect (`actionNurture`/`actionRecap`/`actionReactivate` only ever opened a manual draft modal; `actionFollowUp` additionally bumped `last_contact`, a minor loss). Confirm whether losing the prep-brief trigger is acceptable, or whether it needs a new home.

**Part 5 — form submission → lead history, investigated as asked, not assumed:**

The spec asked whether a submission on "the live Discovery Qualification Form (already connected to HubSpot)" reaches Sales-AI as a visible event today. Findings, checked directly against this repo's code and the connected HubSpot portal (via the CRM MCP connection — not guessed):

1. **This form is real and distinct from `questionnaire.html`.** `questionnaire.html` → `questionnaire_responses` → `qualify-lead` is this repo's own pipeline and has zero HubSpot involvement (grepped both files for `hubspot`, zero hits). The Discovery Qualification Form is a separate, live, HubSpot-native form — confirmed by querying real HubSpot contacts with `recent_conversion_event_name = "Form: Discovery Qualification Form"` and inspecting what it actually writes to a contact: 14 custom properties (`monthly_budget`, `dream_patient`, `practice_overview`, `practice_stage`, `past_agency_experience`, `past_agency_count`, `past_experience_details`, `vision_for_success`, `magic_wand_answer`, `growth_priorities`, `open_to_paid_ads`, `social_media_handler`, `current_marketing`, `monthly_marketing_spend`) plus HubSpot's standard `recent_conversion_event_name`/`recent_conversion_date`.
2. **Before this pass, none of that data ever reached Sales-AI at all** — not "reached silently," genuinely never fetched. `getHubspotContactById` (`_shared/hubspot.ts`) only requested `email,firstname,lastname,company,phone,hs_lead_status,hs_analytics_source,createdate,lastmodifieddate` — none of the form's fields, not even the conversion-tracking ones, were in that list. And even if they had been, `upsertLeadFromHubspotContact` (`_shared/hubspotSync.ts`) only ever fills `leads` gaps (`hubspot_contact_id`/`company_id`) — it has never written to `comm_log` under any circumstance. So the premise in the spec was correct: a submission updated HubSpot with no distinguishable event reaching Sales-AI.
3. **Fixed**: `getHubspotContactById` now requests the form's fields plus `recent_conversion_event_name`/`recent_conversion_date`. New `maybeLogQualificationFormSubmission()` (`_shared/hubspotSync.ts`), called from `hubspot-webhook-receiver` right after every contact upsert, checks whether `recent_conversion_event_name` equals the form's exact name and — if this is a submission newer than `leads.last_qualification_form_submission_at` (schema v14; prevents re-logging the same submission on every later unrelated property edit to that contact) — writes a `comm_log` row (`type: 'qualification_form_submission'`, subject "Qualification form submitted", one line per answer that was actually filled in, dated to the real `recent_conversion_date`) and stamps that column so it won't fire again for the same submission.
4. **What this fix can't do anything about**, flagged rather than papered over: HubSpot's CRM webhooks have no distinct "form submitted" event type — `contact.propertyChange` is generic, so this detects the form by matching `recent_conversion_event_name` against its exact current name. If the form is ever renamed in HubSpot, this silently stops matching (the constant is `QUALIFICATION_FORM_EVENT_NAME` in `hubspotSync.ts` — update it there if that happens). Separately, and pre-existing regardless of this fix: `hubspot-webhook-receiver`'s own webhook subscriptions have never been verified against a live, actually-firing HubSpot App (DEPLOYMENT.md step 8a already flagged this before this pass) — this fix makes the code correct once an event arrives, but doesn't independently confirm HubSpot is currently sending one for this contact/property combination.
